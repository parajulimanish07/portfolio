...
export default function ExperienceSection({ onNavigate }: ExperienceSectionProps) {
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 lg:py-20 flex flex-col justify-center">
      ...
      {/* Timeline Container */}
      <div className="relative pl-5 sm:pl-8 border-l-2 border-slate-200 space-y-6 sm:space-y-7 my-2">
        {experienceData.map((item) => {
          ...
          return (
            <div key={item.id} className="relative group">
              ...
              {/* Experience Card */}
              <div className="p-4 sm:p-6 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 transition-all duration-200 shadow-xs">
                {/* Header row */}
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-slate-100">
                  <span className="text-[10px] font-mono text-slate-400 mr-1 uppercase">Stack:</span>
                  {item.stack.map((tech) => (
                    <span
                      key={tech}
                      className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-100 border border-slate-200/60 text-slate-700 font-medium"
                    >
                      {tech}
                    </span>
                  ))}
